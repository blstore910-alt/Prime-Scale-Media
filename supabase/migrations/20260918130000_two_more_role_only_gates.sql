-- =====================================================================
-- The two role-only gates that matter, named at last
-- =====================================================================
-- ⚠️ APPLY ON SUPABASE MANUALLY.
--
-- The live list of role-only SECURITY DEFINER functions came back as
-- eight. Six of them look at a role to DECIDE rather than to authorise —
-- _guard_user_profile_role, ensure_advertiser_and_wallet,
-- create_admin_for_tenant, create_advertiser_on_profile_insert,
-- create_staff_on_profile_insert (all signup/trigger paths) and
-- wallet_topup_advertiser_create (which gates on owning an advertiser
-- row, not on a role). Those are branches. These two are gates:
--
-- 1. change_subscription_amount — changes what a customer pays every
--    month and can pay a refund into their wallet. Its own check is
--    `up.role = 'admin'` and nothing else, so an admin whose access had
--    been taken away could still reprice a customer and move money.
--
-- 2. _psm_admin_of — a FOURTH admin helper. 20260916100000 patched
--    _is_admin_of, _is_super_admin_of and is_tenant_admin and this one
--    did not exist yet: it was added the next week by
--    20260917120000_user_profiles_admin_update.sql, inheriting the
--    pattern from before the rule changed. Everything built on it — the
--    user_profiles admin policies, and anything added later — therefore
--    still admits a deactivated admin.
--
-- The change_subscription_amount body is generated from 20260917200000 by
-- script, byte-for-byte apart from the predicate: it is 200 lines of
-- refund clamps and voiding logic and every one of those exists because
-- of a real bug.
--
-- ROLLBACK: re-apply 20260917200000 and 20260917120000.
-- =====================================================================
...
as $$
  select exists (
    ...
    where up.user_id = auth.uid()
      ...
      -- sides because "never set" means active.
...
-- ── 2. The subscription-amount gate ──────────────────────────────────
  ...
  if v_uid is not null and not exists (
    select 1 from public.user_profiles up
     where up.user_id = v_uid
       ...
       and up.role = 'admin'
       -- Deactivated keeps the role and loses the access. Without this,
       -- an admin whose access had been removed could still change what a
       -- customer pays every month and trigger a refund into their wallet.
       and coalesce(up.is_active, true) = true
       and coalesce(up.status, 'active') <> 'inactive'
  ) then
...
-- ── Read it back ─────────────────────────────────────────────────────
-- Both true, and the count drops from 8 to 6. The six that remain are the
-- branches listed in the header; supabase/checks/WHICH-8-ARE-ROLE-ONLY.sql
-- names them so that stays a judgement rather than an assumption.
select
  (select p.prosrc ilike '%is_active%' from pg_proc p
     join pg_namespace n on n.oid = p.pronamespace
    where n.nspname = 'public' and p.proname = '_psm_admin_of')              as psm_admin_of_guarded,
  (select p.prosrc ilike '%is_active%' from pg_proc p
     join pg_namespace n on n.oid = p.pronamespace
    where n.nspname = 'public' and p.proname = 'change_subscription_amount') as change_amount_guarded,
