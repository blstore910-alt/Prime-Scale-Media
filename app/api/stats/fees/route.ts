...
// rate has no non-USD money to convert either.
  ...
  const paged = await pageAllRowsTolerant<FeeRow>(
    ...
    supabase
      .from("top_ups")
      .select("created_at, verified_at, currency, fee_amount, topup_usd")
      .eq("tenant_id", profile.tenant_id)
  ...
  const totals = rows.reduce(
    (acc, row) => {
      ...
      acc.count += 1;
      // Same rule as the series above: a customer-filed row is already
      // in its own currency and must not be converted again.
      acc[currency].amount +=
