...
// (/invite/*) stand in the same room as sign-in and sign-up instead
// of a white shadcn card on a blank page. Every class the auth forms
// use (.card .login-card .field .inp .btn .meta ...) lives in AUTH_CSS.
...
.psmauth .status-card .btn svg{width:17px;height:17px}
.psmauth .hint{font-size:.78rem;color:var(--faint);margin:6px 0 0;text-align:left;line-height:1.5}
/* A row per organisation / invitation: pick one. */
.psmauth .orgs{display:grid;gap:8px;margin:0 0 14px;text-align:left}
.psmauth .orgbtn{width:100%;display:flex;align-items:center;gap:11px;padding:11px 12px;border-radius:12px;
  border:1px solid var(--line-2);background:var(--panel);cursor:pointer;font:inherit;color:var(--ink);text-align:left;transition:.14s}
.psmauth .orgbtn:hover{border-color:var(--primary);box-shadow:0 0 0 3px var(--primary-tint)}
.psmauth .orgbtn:disabled{opacity:.6;cursor:default}
.psmauth .orgbtn .av{width:34px;height:34px;border-radius:10px;display:grid;place-items:center;flex:0 0 auto;
  background:var(--brand);color:#fff;font-family:var(--hd);font-weight:800;font-size:.85rem}
.psmauth .orgbtn .nm{flex:1;min-width:0}
...
@media(max-width:860px){
  ...
  .psmauth .orgbtn{background:rgba(255,255,255,.06);border-color:rgba(255,255,255,.16);color:#fff}
  .psmauth .orgbtn:hover{box-shadow:0 0 0 3px rgba(91,141,255,.2)}
  .psmauth .orgbtn .nm small{color:rgba(255,255,255,.55)}
  .psmauth .orgbtn .go{color:#9db8ff}
  .psmauth .hint{color:rgba(255,255,255,.55)}
