...
type AffiliateCommissionsStatsResponse = {
...
async function fetchAffiliateCommissionsStats(
  period: DashboardPeriod,
  dateRange?: DashboardDateRange,
): Promise<AffiliateCommissionsStatsResponse> {
  const { from, to } = getPeriodRange(period, dateRange);
  const searchParams = new URLSearchParams({ from, to });
  const res = await fetch(
    `/api/stats/affiliate-commissions?${searchParams.toString()}`,
  );
  if (!res.ok) {
    throw new Error("Failed to fetch affiliate commissions stats");
  }
  return res.json();
}
...
export function AffiliateCommissionsStatsCard({
  period,
  dateRange,
}: {
  period: DashboardPeriod;
  ...
  const { data, isLoading, isError } = useQuery({
    ...
    queryFn: () => fetchAffiliateCommissionsStats(period, dateRange),
    staleTime: 1000 * 60 * 5,
  });
    ...
    <Card className="@container/card gap-2 py-4">
      <CardHeader className="pb-2">
        <CardDescription className="font-semibold text-card-foreground">
          Affiliate Commissions <span>({formatNumber(data.totals.count)})</span>
        </CardDescription>
                    ...
                    return (
                      <ChartTooltipContent
                        ...
                        formatter={(value, name, item) => {
                          const currentPoint =
                            item.payload as CommissionSeriesPoint;
