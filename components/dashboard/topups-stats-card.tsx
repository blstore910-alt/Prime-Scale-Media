...
async function fetchTopupsStats(
  period: DashboardPeriod,
  dateRange?: DashboardDateRange,
): Promise<TopupsStatsResponse> {
  const { from, to } = getPeriodRange(period, dateRange);
  const searchParams = new URLSearchParams({ from, to });
  const res = await fetch(`/api/stats/topups?${searchParams.toString()}`);
  if (!res.ok) {
    throw new Error("Failed to fetch topup stats");
  }
  return res.json();
}
...
export function TopupsStatsCard({
  period,
  dateRange,
}: {
  period: DashboardPeriod;
  ...
  const { data, isLoading, isError } = useQuery({
    ...
    queryFn: () => fetchTopupsStats(period, dateRange),
    staleTime: 1000 * 60 * 5,
  });
