...
/* Swipe-to-archive and the Inbox/Archive tabs moved to
   components/notifications/notifications-css.ts. /notifications is a
   SHARED route and only the admin shell injects this file, so those
   rules reached admins and nobody else -- and a customer opening their
   own alerts got an unstyled, broken layout. The page carries them now.

   A BASE .seg2 though, because PSM_APP_CSS never had one: the rule
   lives in the advertiser and affiliate shells only, so the manual
   page's audience switch rendered as plain grey browser buttons with
   no indication of which one was selected. */
.psmapp .seg2{display:flex;flex-wrap:wrap;gap:2px;padding:3px;border-radius:10px;
  background:var(--panel-2);border:1px solid var(--line)}
.psmapp .seg2 button{border:0;background:none;font-family:var(--bd);font-weight:700;
  font-size:.82rem;color:var(--txt-2);padding:7px 12px;border-radius:8px;
  cursor:pointer;transition:.13s;white-space:nowrap}
.psmapp .seg2 button:hover{color:var(--ink)}
.psmapp .seg2 button.on{background:var(--panel);color:var(--primary-600);
